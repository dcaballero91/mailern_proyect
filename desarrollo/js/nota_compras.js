...
var ordencompras = $('#ordencompras').dataTable( {
   ...
   $('#ordencompras tbody').on( 'click', 'tr td.details-control', function () {        
    ...
    $(document).on("click","#grabar",function(){
        ...
        if(fechafactura > timbradovighasta ){
            humane.log("<span class='fa fa-info'></span>La fecha de la factura no puede ser mayor a la fecha de vencimiento del timbrado", { timeout: 4000, clickToClose: true, addnCls: 'humane-flatty-warning' });
            return
        }
        ...
        if(tipo_nota === 'DEBITO' || (tipo_nota === 'CREDITO' && tipo_mot_nota === 'DESCUENTO')){
            if(notamonto>0 && desc!=="" && compcod>0 && nrofactura !=="" && tipo_nota!=="" && timbrado !=="" && timbradovighasta !==""
                && fechafactura !=="" ){
                $.ajax({
                type: "POST",
                url: "grabar.php",
                data: {notanro:0, compcod:compcod, fechafactura:fechafactura, timbrado:timbrado, timbradovighasta:timbradovighasta, nrofactura:nrofactura, tipo_nota:tipo_nota, tipo_mot_nota:tipo_mot_nota, suc:suc,usu:usu, notamonto:notamonto, desc:desc, detalle:'{}', ope:1}
            ...
            if(detalle!=="{}" && compcod > 0 && nrofactura !== "" && tipo_nota!=="" && timbrado !=="" && timbradovighasta !==""
                && fechafactura !=="" ){
                $.ajax({
                type: "POST",
                url: "grabar.php",
                data: {notanro:0, compcod:compcod, fechafactura:fechafactura, timbrado:timbrado, timbradovighasta:timbradovighasta, nrofactura:nrofactura, tipo_nota:tipo_nota, tipo_mot_nota:tipo_mot_nota, suc:suc, usu:usu, notamonto:0, desc:desc, detalle:detalle, ope:1}
                }).done(function(msg){
                    mostrarMensajes(msg);
